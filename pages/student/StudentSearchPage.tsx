...
const StudentSearchPage: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (success) navigate(`/student/${studentId.trim()}`);
        else setError('ข้อมูลไม่ถูกต้อง');
    } catch (err: any) { setError(err.message); }
  };

  const handleLineLogin = () => {
      if (lineLoginUrl) window.location.href = lineLoginUrl + '&state=student';
      ...
      <div className="w-full max-w-sm text-center">
        ...
        <div className="bg-white/50 backdrop-blur-lg p-8 rounded-3xl shadow-lg">
          <form onSubmit={handleLogin} className="space-y-4">
            <input value={studentId} onChange={e => setStudentId(e.target.value)} placeholder="รหัสนักเรียน" className="w-full px-4 py-3 bg-white/70 border border-sky-200 rounded-xl" required />
            <input value={email} onChange={e => setEmail(e.target.value)} placeholder="อีเมล" className="w-full px-4 py-3 bg-white/70 border border-sky-200 rounded-xl" required />
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="รหัสผ่าน" className="w-full px-4 py-3 bg-white/70 border border-sky-200 rounded-xl" required />
            {error && <p className="text-red-500 text-sm">{error}</p>}
            <button type="submit" disabled={loading} className="w-full bg-sky-500 text-white font-bold py-3 px-4 rounded-xl shadow-md hover:bg-sky-600">{loading ? '...' : 'เข้าสู่ระบบ'}</button>
          </form>
          <div className="mt-4">
          ...
          <p className="text-sm text-slate-600 mt-6">ยังไม่มีบัญชี? <Link to="/student/register" className="font-semibold text-sky-600 hover:underline">ลงทะเบียน</Link></p>
          <button onClick={() => navigate('/')} className="mt-4 text-sm text-slate-500 hover:text-slate-700">กลับหน้าหลัก</button>
        </div>
      </div>
    </div>
  );
