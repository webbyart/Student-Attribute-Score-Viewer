...
const DayEventsModal: React.FC<DayEventsModalProps> = ({ date, tasks, onClose, onTaskClick }) => {
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in" onClick={onClose}>
      <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[70vh]" onClick={e => e.stopPropagation()}>
         ...
         <div className="p-4 overflow-y-auto space-y-3 bg-slate-50 flex-1">
            {tasks.length > 0 ? (
                tasks.map(task => (
                    <div key={task.id} onClick={() => onTaskClick(task)} className="bg-white p-3 rounded-xl shadow-sm border border-slate-100 active:scale-95 transition cursor-pointer hover:shadow-md">
